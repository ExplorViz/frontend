...
export type AuxiliaryScrollMenuArgs = BaseMenuArgs & {
    ...
    //scrollArea: DetailInfoScrollarea,
...
export class AuxiliaryScrollMenu extends BaseMenu {
    ...
    constructor({text, /*scrollArea,*/ controller, renderer, object, ...args} : AuxiliaryScrollMenuArgs){
        super(args);
        this.text = text;
    ...
    return new VRControllerButtonBinding('', {
      onButtonPress: () => {
       ...
       // this.controller?.gripIsPressed = true;
        ...
        this.closeMenu();
        if(this.controller){
            this.renderer.grabIntersectedObject(this.controller);
            this.controller.updateIntersectedObject();
