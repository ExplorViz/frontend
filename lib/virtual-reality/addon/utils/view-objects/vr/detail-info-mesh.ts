...
export default class DetailInfoMesh extends ThreeMeshUI.Block /*implements IntersectableObject*/ {
    ...
    constructor(content : string, options : ThreeMeshUI.BlockOptions){
        ...
        const textBlockBlock = new ThreeMeshUI.Block({width: 0.65, height: 0.45, hiddenOverflow: true, backgroundOpacity: 0, offset: 0.001,}); // This Block with hiddenOverflow set to true is needed so the text stays within its block!
        this.text = new ThreeMeshUI.Text({content: this.content, backgroundOpacity: 0});
        const textBlock = new DetailInfoScrollarea(this.text, {height: 0.45, width: 0.65, backgroundOpacity: 0, offset: 0.001,});
        const scrollButtonBox = new ThreeMeshUI.Block({height: 0.1, width: 0.65, contentDirection: 'row', backgroundOpacity: 0, offset: 0.001,});
