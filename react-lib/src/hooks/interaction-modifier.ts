...
export default function useInteractionModifier(
  ...
  const minimapState = useMinimapStore(
    useShallow((state) => ({
      makeFullsizeMinimap: state.makeFullsizeMinimap,
  ...
  const collaborationState = useCollaborationSessionStore(
    useShallow((state) => ({
  ...
  const raycastObjects = useRef<Object3D[]>(
    objectsToRaycast instanceof Object3D ? [objectsToRaycast] : objectsToRaycast
  );
  const camera = useRef<THREE.Camera>(cameraToUse);

  // Update these ref values to function parameters on every render pass
  // (such that event handlers needn't be re-added)

  raycastObjects.current =
    objectsToRaycast instanceof Object3D
      ? [objectsToRaycast]
      : objectsToRaycast;

  camera.current = cameraToUse;
  ...
  const onPointerMove = (event: PointerEvent) => {
    ...
    } else if (minimapState.makeFullsizeMinimap) {
      const intersectedViewObj = minimapState.raycastForObjects(
        event,
        minimapCamera,
        raycastObjects.current
      );
  ...
  const onWheel = (event: WheelEvent) => {
    if (minimapState.isMouseInsideMinimap(event)) {
      minimapState.addZoomDelta(-event.deltaY / 1000);
    }
  };
  ...
  const onLeftClick = (
    ...
    const isOnMinimap = minimapState.isMouseInsideMinimap(event);
    const rayMarkers = minimapState.raycastForMarkers(event);
    // if rayMarkers are present, it means that the click was on a marker
    if (rayMarkers) {
      handleMinimapOnLeftClick(isOnMinimap, rayMarkers);
      return;
    } else if (minimapState.makeFullsizeMinimap && isOnMinimap) {
      const rayObjects = minimapState.raycastForObjects(
        event,
        minimapCamera,
        raycastObjects.current
      );
      ...
      minimapState.toggleFullsizeMinimap(false);
      return;
    } else if (isOnMinimap) {
      handleMinimapOnLeftClick(isOnMinimap, rayMarkers);
      return;
  ...
  const handleMinimapOnLeftClick = (
    ...
    if (minimapState.makeFullsizeMinimap && !isOnMinimap) {
      minimapState.toggleFullsizeMinimap(false);
    } else if (isOnMinimap) {
      if (ray) {
        minimapState.handleHit(
          collaborationState.getUserById(ray.object.name) as RemoteUser
        );
      } else {
        minimapState.toggleFullsizeMinimap(true);
      }
    }
  };
  ...
  const handleMinimapDoubleClick = (event: MouseEvent) => {
    if (minimapState.isMouseInsideMinimap(event)) {
      return minimapState.raycastForObjects(
        event,
        minimapCamera,
        raycastObjects.current
      );
  ...
  const ping = (intersectedViewObj: THREE.Intersection | null) => {
    if (intersectedViewObj) {
      localUserState.ping(intersectedViewObj.object, intersectedViewObj.point);
    }
  };
  ...
  const createPointerStopEvent = () => {
    if (!canvas.current) {
      console.error('Cannot create custom MouseStop event: Canvas ref not set');
      return;
    }
  ...
  const handleLongPress = (event: PointerEvent) => {
    ...
    } else if (longPressDelta.current.x < 35 && longPressDelta.current.y < 35) {
      dispatchOpenMenuEvent(event);
    }
  };
  ...
  useEffect(
    function updateRaycasterMinimapCam() {
