...
export default function CommitTreeApplicationSelection({
    ...
    if (appNameCommitTreeMap.size > 0) {
        const selAppName = selectedAppName ? selectedAppName != undefined : "Applications";
        ...
        let dropdownMenu = obj.array.map((item) =>
            <div
                ...
                onClick={() => useCommitTreeStateStore.getState().setCurrentSelectedApplicationName(item)}
                >
                {item}
            </div>
        );
        ...
        return (
            <div className='col-md-auto'>
                <div id='application-selection' className='dropdown' style={{zIndex: 500}}>
                <button
