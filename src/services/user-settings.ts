import isObject, { objectsHaveSameKeys } from '../utils/object-helpers';
import {
...
} from '../utils/settings/color-schemes';
import {
  defaultApplicationSettings,
  defaultLanscapeSettings,
} from '../utils/settings/default-settings';
import {
...
export default class UserSettings {
  ...
  private restoreLandscapeSettings() {
    ...
    if (this.areValidLandscapeSettings(parsedLandscapeSettings)) {
      this.landscapeSettings = parsedLandscapeSettings;
    } else {
  ...
  private restoreApplicationSettings() {
    ...
    if (this.areValidApplicationSettings(parsedApplicationSettings)) {
      this.applicationSettings = parsedApplicationSettings;
    } else {
  ...
  applyDefaultLandscapeSettings() {
    this.landscapeSettings = JSON.parse(
      JSON.stringify(defaultLanscapeSettings)
    );
  }
