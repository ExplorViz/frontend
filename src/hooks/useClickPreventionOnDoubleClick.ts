...
export const CLICK_PREVENTION_DEFAULTS = {
  ...
  /** Max pointer movement (px) treated as a click rather than a drag/pan. */
  allowedDelta: 12,
} as const;

/** Tracks max pointer movement (px) from `nativeEvent` until `cleanup` runs. */
export function maxPointerDriftDuringDelay(
  ...
  const handleClick = (
    ...
    return waitForClick.promise
      .then(() => {
        ...
        const pointerDrift = getMaxDistance();
        ...
        try {
          if (pointerDrift < allowedDelta && e.delta < allowedDelta) {
            onClick(e);
          }
          // Always execute click event if delta cannot be determined
        } catch {
          if (pointerDrift < allowedDelta) {
            onClick(e);
          }
        }
      })
