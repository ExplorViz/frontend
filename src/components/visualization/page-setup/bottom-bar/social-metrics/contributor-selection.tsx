...
export default function ContributorSelection() {
  ...
  const isBot = (c: ContributorDto): boolean => {
    const name = (c.githubLogin ?? c.gitUsername ?? c.email ?? '').toLowerCase();
  ...
  const selectPreset = (preset: 'all' | 'none' | 'core' | 'edge' | 'human') => {
    switch (preset) {
      ...
      case 'human': return setSelectedContributorIds(getIds(contributors.filter((c) => !isBot(c))))
    }
  }

  return (
    <div className="social-metrics-control-block">
      <span className="social-metrics-control-label">Presets
        <HelpTooltip
      ...
      <div className="social-metrics-preset-group">
        <Button size="sm" variant="outline-secondary" title="select everyone" onClick={() => selectPreset('all')}>All</Button>
        <Button size="sm" variant="outline-secondary" title="clear selection" onClick={() => selectPreset('none')}>None</Button>
        <Button size="sm" variant="outline-secondary" title="Top contributors making >= 80% of all commits" onClick={() => selectPreset('core')}>Core</Button>
        <Button size="sm" variant="outline-secondary" title="Everyone outside of core" onClick={() => selectPreset('edge')}>Edge</Button>
        <Button size="sm" variant="outline-secondary" title="Excludes any names containing “BOT”" onClick={() => selectPreset('human')}>Human</Button>
      </div>
