...
export default function CollaborationComponentSync() {
  ...
  const localHiddenBuildings = useVisualizationStore(
    (state) => state.hiddenBuildingIds
  );
  ...
  useEffect(() => {
    ...
    if (isFirstRender.current) {
      ...
      if (
        globalString !==
        JSON.stringify({ closed: [], hiddenDistricts: [], hiddenBuildings: [] })
      ) {
