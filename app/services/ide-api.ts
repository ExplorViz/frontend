...
function getOrderedParents(dataModel: Application): ParentOrder {
  let result: ParentOrder = { fqn: dataModel.name, childs: [], meshid: dataModel.id }
  let temp: ParentOrder[] = [];
  dataModel.packages.forEach(element => {
    let fqn = dataModel.name + "." + element.name
    temp.push({ fqn: fqn, childs: parentPackage(fqn, element.subPackages, element.classes), meshid: element.id })
...
function parentPackage(fqn: string, subpackages: Package[], classes: Class[]): ParentOrder[] {
  let temp: ParentOrder[] = [];
  ...
  subpackages.forEach(element => {
    ...
    temp.push({ fqn: newFqn, childs: parentPackage(newFqn, element.subPackages, element.classes), meshid: element.id })
  });
...
function getFqnForMeshes(orderedParents: ParentOrder): { meshNames: string[]; meshIds: string[]; } {
  ...
  orderedParents.childs.forEach(element => {
    meshTemp.meshNames = meshTemp.meshNames.concat(getFqnForMeshes(element).meshNames)
    meshTemp.meshIds = meshTemp.meshIds.concat(getFqnForMeshes(element).meshIds)
  });
...
function OpenObject(doSomethingOnMesh: (mesh: THREE.Object3D) => void, fullQualifiedName: string, occurrenceID: number, lookAtMesh: (mesh: THREE.Object3D) => void, appli3DObj: ApplicationObject3D[]) {
  ...
  appli3DObj.forEach(element => {
    ...
    console.log(element.dataModel.name + occurrenceName + fullQualifiedName, orderTuple, element)
    recursivelyOpenObjects(doSomethingOnMesh, lookAtMesh, element.dataModel.name + occurrenceName + fullQualifiedName, orderTuple, element)

  });
}
function resetFoundation(doSomethingOnMesh: (mesh: THREE.Object3D) => void, appli3DObj: ApplicationObject3D, orderTuple: OrderTuple) {
  let mesh = appli3DObj.children[orderTuple.meshes.meshNames.indexOf(orderTuple.hierarchyModel.fqn)]
  console.log(appli3DObj)
  doSomethingOnMesh(mesh)

}

function recursivelyOpenObjects(doSomethingOnMesh: (mesh: THREE.Object3D) => void, lookAtMesh: (mesh: THREE.Object3D) => void, toOpen: string, orderTuple: OrderTuple, appli3DObj: ApplicationObject3D) {
  ...
  // }
    ...
    orderTuple.hierarchyModel.childs.forEach(element => {
      let tempOrder: ParentOrder = { fqn: element.fqn, childs: element.childs, meshid: element.meshid };
      if (isInParentOrder(element, toOpen)) {
        console.log("DoSome:", element)
        doSomethingOnMesh(appli3DObj.children[orderTuple.meshes.meshNames.indexOf(element.fqn)])
        lookAtMesh(appli3DObj.children[orderTuple.meshes.meshNames.indexOf(element.fqn)])
...
function isInParentOrder(po: ParentOrder, name: string) {
  ...
  po.childs.forEach(element => {
    tempBool = tempBool || isInParentOrder({ fqn: element.fqn, childs: element.childs, meshid: element.meshid }, name)
  });
