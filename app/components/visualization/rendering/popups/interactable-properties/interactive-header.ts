...
export default class InteractiveHeader extends Component<Args> {
    ...
    save() {
        ...
        if(isApplication(this.args.entity))
            this.landscapeRestructure.updateApplicationName(this.name, this.args.entity.id);
        else if(isPackage(this.args.entity))
            this.landscapeRestructure.updatePackageName(this.name, this.args.entity.id);
        else if(isClass(this.args.entity))
            this.landscapeRestructure.updateClassName(this.name, this.args.entity.id, this.args.appId);
